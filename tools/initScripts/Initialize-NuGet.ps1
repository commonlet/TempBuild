...
New-Item -ItemType Directory -Force -Path "$RepoRoot\tools\nuget" | Out-Null
$nugetDir = Join-Path -Resolve $RepoRoot "tools\nuget"

# Ensure nuget.exe is up-to-date
$nugetDownloadName = "nuget.exe"
$nuget_exe = . "$PSScriptRoot\Initialize-DownloadLatest.ps1" -OutDir $nugetDir -DownloadUrl "https://dist.nuget.org/win-x86-commandline/latest/nuget.exe" -DownloadName $nugetDownloadName -Unzip $false
